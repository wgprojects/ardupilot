...
#define THISFIRMWARE "ArduSkipper v0.1"
/*
...
/* 
   This is the APMsailboat firmware. It was originally derived from
   APMrover2 firmware, which was originally derived from
   ArduPlane by Jean-Louis Naudin (JLN), and then rewritten after the
   AP_HAL merge by Andrew Tridgell

   Maintainer: N/A

   Authors:    William Gibson, Doug Weibel, Jose Julio, Jordi Munoz, Jason Short, Andrew Tridgell, Randy Mackay, Pat Hickey, John Arne Birkeland, Olivier Adler, Jean-Louis Naudin
...
#include <AP_Anemometer.h>		//TODO implement the wind direction (optionally wind speed) sensor.
#include <AP_Baro.h>
...
static RC_Channel *channel_sail;
...
/*
  check for new anemometer data - 10Hz
 */
static void update_anemometer(void)
{
    if (g.anemometer_enabled && anemometer.read()) {
        
        if (should_log(MASK_LOG_ANEMOMETER)) {
            Log_Write_Anemometer();
        }
    } else {
...
{ 
    ...
    case STEERING: {
		...
		channel_sail->servo_out = channel_sail->control_in;
        break;
    }
    ...
    case HOLD:
		...
		//TODO hacksail - go into irons -> PID control heading into the wind
