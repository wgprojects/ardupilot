...
def sim_recv(state):
    ...
    state.sail = (2012-pwm[4])/828.0
    # scale centi-units to full units
    state.wind_speed = speed * 0.01;
    state.wind_dir = direction * 0.01;

#    print("steering=%f sail=%f pwm=%s" % (state.steering, state.sail, str(pwm)))
...
parser = OptionParser("sim_sailboat.py [options]")
